...
import static org.openhab.binding.linkyd2l.internal.LinkyD2LBindingConstants.*;
...
import org.openhab.binding.linkyd2l.internal.LinkyD2LException;
...
public class LinkyD2LHandler extends BaseThingHandler {
    ...
    private void checkData(Consumption consumption) throws LinkyD2LException {
        if (consumption.aggregats.days.periodes.size() == 0) {
            throw new LinkyD2LException("invalid consumptions data: no day period");
        }
        if (consumption.aggregats.days.periodes.size() != consumption.aggregats.days.datas.size()) {
            throw new LinkyD2LException("invalid consumptions data: not one data for each day period");
        }
        if (consumption.aggregats.weeks.periodes.size() == 0) {
            throw new LinkyD2LException("invalid consumptions data: no week period");
        }
        if (consumption.aggregats.weeks.periodes.size() != consumption.aggregats.weeks.datas.size()) {
            throw new LinkyD2LException("invalid consumptions data: not one data for each week period");
        }
        if (consumption.aggregats.months.periodes.size() == 0) {
            throw new LinkyD2LException("invalid consumptions data: no month period");
        }
        if (consumption.aggregats.months.periodes.size() != consumption.aggregats.months.datas.size()) {
            throw new LinkyD2LException("invalid consumptions data: not one data for each month period");
        }
        if (consumption.aggregats.years.periodes.size() == 0) {
            throw new LinkyD2LException("invalid consumptions data: no year period");
        }
        if (consumption.aggregats.years.periodes.size() != consumption.aggregats.years.datas.size()) {
            throw new LinkyD2LException("invalid consumptions data: not one data for each year period");
        }
    }
